...
export default function Home(){
  ...
  const [searchQuery,setSearchQuery]=useState(""),[searchFilter,setSearchFilter]=useState<"all"|"teams"|"players"|"prospects"|"topics">("all"),[trending,setTrending]=useState<TrendingDebate>(featuredTrending()),[trendingLoading,setTrendingLoading]=useState(false),[searchGenerating,setSearchGenerating]=useState(false);
  ...
  async function loadProspectSystem(){
    if(!prospectSystem||prospectLoading)return;
    setProspectLoading(true);setProspectError("");
    const cacheKey=`sports-debate-prospects-${prospectSystem.league}-${prospectSystem.team}`;
    try{
      const cached=localStorage.getItem(cacheKey);
      if(cached){const parsed=JSON.parse(cached);if(parsed?.savedAt&&Date.now()-parsed.savedAt<7*86400000&&Array.isArray(parsed.prospects)){setProspectResults(parsed.prospects);setProspectLoading(false);return}}
      const data=await api({action:"prospectSystem",team:prospectSystem.team,league:prospectSystem.league});
      const prospects=(data.prospects||[]).map((p:any,i:number)=>({rank:Number(p.rank)||i+1,name:String(p.name),position:p.position||"",currentLevel:p.currentLevel||"",team:prospectSystem.team,league:prospectSystem.league,sport:prospectSystem.sport,take:p.take||`${p.name} should be considered ${prospectSystem.team}'s most important prospect.`}));
      setProspectResults(prospects);localStorage.setItem(cacheKey,JSON.stringify({savedAt:Date.now(),prospects}));
    }catch(e){setProspectError("Could not load the current rankings. Try again in a moment.")}finally{setProspectLoading(false)}
  }
