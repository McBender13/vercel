...
type TrendingDebate = { sport:string; take:string; context?:string; sources?:Source[]; plays?:number; likes?:number; createdAt?:string };
...
type SavedDebate = { id:string; createdAt:string; sport:string; take:string; difficulty:Difficulty; side:Side; mode?:DebateMode; kind?:DebateKind; result:"win"|"loss"|"draw"; userTotal:number; aiTotal:number; exchanges:Exchange[]; eloBefore?:number; eloAfter?:number; eloChange?:number };
...
const TRENDING_ACTIVITY_KEY="sports-debate-trending-activity-v1";
...
const FEATURED_TRENDING: TrendingDebate[] = [
  {sport:"Basketball",take:"Teams should prioritize roster depth over adding a third superstar.",context:"A rotating featured debate inspired by the biggest roster-building arguments in sports.",plays:820,likes:214,createdAt:"2026-07-19T12:00:00.000Z"},
  {sport:"Hockey",take:"Teams should be more willing to trade first-round picks at the deadline.",context:"A rotating featured debate about balancing championship windows and future assets.",plays:640,likes:176,createdAt:"2026-07-18T12:00:00.000Z"},
  {sport:"Soccer",take:"Clubs should value tactical fit more than star power in the transfer market.",context:"A rotating featured debate about modern squad building.",plays:950,likes:301,createdAt:"2026-07-20T12:00:00.000Z"},
  {sport:"Football",take:"Paying an elite quarterback top-market money makes it harder to build a champion.",context:"A rotating featured debate about salary-cap team building.",plays:780,likes:229,createdAt:"2026-07-17T12:00:00.000Z"},
  {sport:"Baseball",take:"Contending teams should be aggressive buyers even when the prospect cost is high.",context:"A rotating featured debate about the trade deadline and championship windows.",plays:510,likes:142,createdAt:"2026-07-16T12:00:00.000Z"}
];
...
export default function Home(){
  ...
  const [lastEloChange,setLastEloChange]=useState(0),[lastEloAfter,setLastEloAfter]=useState(1000);
  const [trendingSport,setTrendingSport]=useState("All"),[trendingSort,setTrendingSort]=useState<"played"|"liked"|"newest">("played"),[trendingSearch,setTrendingSearch]=useState("");
  const [trendingActivity,setTrendingActivity]=useState<Record<string,{plays:number;likes:number}>>({});
  ...
  const stats=useMemo(()=>{const wins=history.filter(h=>h.result==="win").length,losses=history.filter(h=>h.result==="loss").length,draws=history.filter(h=>h.result==="draw").length,total=history.length;let currentStreak=0,bestStreak=0,run=0;for(const h of [...history].reverse()){if(h.result==="win"){run++;bestStreak=Math.max(bestStreak,run)}else run=0}for(const h of history){if(h.result==="win")currentStreak++;else break}const rounds=history.reduce((n,h)=>n+h.exchanges.length,0);const favorite=Object.keys(TAKES).sort((a,b)=>history.filter(h=>h.sport===b).length-history.filter(h=>h.sport===a).length)[0]||"—";const favoriteDifficulty=(['easy','medium','hard','impossible'] as Difficulty[]).sort((a,b)=>history.filter(h=>h.difficulty===b).length-history.filter(h=>h.difficulty===a).length)[0]||"—";let rating=1000,highestElo=1000;for(const h of [...history].reverse()){if(typeof h.eloAfter==='number'){rating=h.eloAfter;highestElo=Math.max(highestElo,h.eloAfter);continue}const multiplier={easy:0,medium:5,hard:10,impossible:18}[h.difficulty]||0;const margin=Math.max(-3,Math.min(3,h.userTotal-h.aiTotal));rating+=h.result==="win"?20+multiplier+margin:h.result==="loss"?-(16+Math.round(multiplier/2)-margin):3;highestElo=Math.max(highestElo,rating)}const userRoundPoints=history.reduce((n,h)=>n+h.exchanges.reduce((a,e)=>a+(e.userScore||0),0),0);const persuasiveness=rounds?Math.round(userRoundPoints/(rounds*10)*100):0;const avgScore=total?Math.round(history.reduce((n,h)=>n+h.userTotal,0)/total*10)/10:0;const avgRound=rounds?Math.round(userRoundPoints/rounds*10)/10:0;const levels:Difficulty[]=["easy","medium","hard","impossible"];const hardest=[...levels].reverse().find(level=>history.some(h=>h.result==="win"&&h.difficulty===level))||"—";return {wins,losses,draws,total,currentStreak,bestStreak,rounds,favorite,favoriteDifficulty,winRate:total?Math.round(wins/total*100):0,rating:Math.max(100,Math.round(rating)),highestElo:Math.max(100,Math.round(highestElo)),persuasiveness,avgScore,avgRound,hardest}},[history]);
  const trendingItems=useMemo(()=>{const merged=[trending,...FEATURED_TRENDING,...trendingQueueRef.current];const unique=new Map<string,TrendingDebate>();for(const item of merged){if(!unique.has(item.take))unique.set(item.take,item)}const q=trendingSearch.trim().toLowerCase();return [...unique.values()].map(item=>{const activity=trendingActivity[item.take]||{plays:0,likes:0};return {...item,plays:(item.plays||0)+activity.plays,likes:(item.likes||0)+activity.likes}}).filter(item=>(trendingSport==="All"||item.sport===trendingSport)&&(!q||`${item.sport} ${item.take}`.toLowerCase().includes(q))).sort((a,b)=>trendingSort==="liked"?(b.likes||0)-(a.likes||0):trendingSort==="newest"?new Date(b.createdAt||0).getTime()-new Date(a.createdAt||0).getTime():(b.plays||0)-(a.plays||0));},[trending,trendingSport,trendingSort,trendingSearch,trendingActivity]);
  ...
  function updateTrendingActivity(item:TrendingDebate,field:"plays"|"likes"){const next={...trendingActivity,[item.take]:{plays:trendingActivity[item.take]?.plays||0,likes:trendingActivity[item.take]?.likes||0}};next[item.take][field]+=1;setTrendingActivity(next);localStorage.setItem(TRENDING_ACTIVITY_KEY,JSON.stringify(next))}
  ...
  function finish(){const final=exchanges.reduce((a,e)=>({user:a.user+(e.userScore||0),ai:a.ai+(e.aiScore||0)}),{user:0,ai:0}),result:SavedDebate["result"]=final.user>final.ai?"win":final.user<final.ai?"loss":"draw";const eloBefore=stats.rating;const difficultyBonus={easy:0,medium:4,hard:8,impossible:14}[difficulty];const margin=Math.max(-3,Math.min(3,final.user-final.ai));const eloChange=result==="win"?18+difficultyBonus+margin:result==="loss"?-(14+Math.round(difficultyBonus/2)-margin):2;const eloAfter=Math.max(100,eloBefore+eloChange);const saved:SavedDebate={id:crypto.randomUUID(),createdAt:new Date().toISOString(),sport,take,difficulty,side,mode,kind,result,userTotal:final.user,aiTotal:final.ai,exchanges,eloBefore,eloAfter,eloChange};setLastEloChange(eloChange);setLastEloAfter(eloAfter);persist([saved,...history].slice(0,100));setStage("finished");play(result==="win"?900:result==="loss"?240:500,260)}
  ...
  return <main className="container">
    ...
    {stage==="finished"&&<div className="card"><div className="eyebrow">FINAL DECISION</div><div className="scoreline">You {totals.user} — {totals.ai} AI</div><h2>{totals.user>totals.ai?"You win":totals.user<totals.ai?"AI wins":"Draw"}</h2><div className="result-meta"><span className={`badge ${lastEloChange>=0?"winner-badge":"loser-badge"}`}>ELO {lastEloChange>=0?"+":""}{lastEloChange} · {lastEloAfter}</span><span className="badge">Record {stats.wins}-{stats.losses}</span><span className="badge">Streak {stats.currentStreak}</span></div>{kind==="challenge"&&challengeTarget&&<div className="tip" style={{marginTop:14}}><strong>{totals.user>challengeTarget?"You beat your friend’s score!":totals.user===challengeTarget?"You tied your friend’s score.":"Your friend keeps the higher score."}</strong><div className="small" style={{marginTop:5}}>Friend target: {challengeTarget} · Your score: {totals.user}</div></div>}<div className="row" style={{marginTop:16}}><button className="btn" onClick={shareChallenge}>Challenge a friend</button><button className="btn secondary" onClick={shareText}>Share transcript</button><button className="btn secondary" onClick={downloadText}>Download text</button>{totals.user>totals.ai&&<button className="btn secondary" onClick={downloadImage}>Share victory card</button>}<button className="btn ghost" onClick={reset}>New debate</button></div></div>}
    </section>}
  </main>
}
